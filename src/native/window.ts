...
export async function createMainWindow() {
  ...
  session.defaultSession.setDisplayMediaRequestHandler(
    (request, callback) => {
      // Loopback audio (system audio during screen share) is only supported on
      // Windows. Passing `audio: "loopback"` on Linux/macOS makes `getDisplayMedia`
      // reject, which aborts the screen share and can disconnect the caller.
      // See Vesktop's implementation for reference.
      const getStreams = (
        video: Electron.DesktopCapturerSource,
        audio: boolean = request.audioRequested,
      ) => {
        const streams: Electron.Streams = { video };
      ...
      desktopCapturer
          ...
          ipcMain.once(
            ...
            (_, idx: number, audio: boolean) => {
              if (idx < 0 || idx > sources.length) {
                ...
                callback(getStreams(sources[idx], audio));
              }
            },
          );
        ...
        .catch((error) => {
          console.error("Failed to get screen share sources", error);
          callback({});
        });
    },
